...
-- ============================================
-- 3. TRANSACCIONES/RELACIONES (MODIFICADO PARA REPORTES)
-- ============================================

-- A. ÓRDENES (Creamos volumen para probar los rangos de lealtad)

-- 1. ADA LOVELACE (ID 1) -> SERÁ VIP (12 Órdenes)
INSERT INTO ordenes (usuario_id, total, status) VALUES
    (1, 1389.97, 'entregado'), (1, 50.00, 'entregado'), (1, 20.00, 'entregado'),
    (1, 100.00, 'entregado'), (1, 45.00, 'entregado'), (1, 120.00, 'entregado'),
    (1, 300.00, 'enviado'),    (1, 15.00, 'enviado'),    (1, 90.00, 'pagado'),
    (1, 200.00, 'pagado'),     (1, 60.00, 'pagado'),     (1, 110.00, 'pendiente');

-- 2. ALAN TURING (ID 2) -> SERÁ LEAL (8 Órdenes)
INSERT INTO ordenes (usuario_id, total, status) VALUES
    (2, 69.98, 'enviado'),  (2, 40.00, 'entregado'), (2, 25.00, 'entregado'),
    (2, 80.00, 'entregado'),(2, 55.00, 'pagado'),    (2, 90.00, 'pagado'),
    (2, 30.00, 'pendiente'),(2, 70.00, 'pendiente');

-- 3. GRACE HOPPER (ID 3) -> SERÁ FRECUENTE (5 Órdenes)
INSERT INTO ordenes (usuario_id, total, status) VALUES
    (3, 284.98, 'pagado'), (3, 40.00, 'entregado'), (3, 150.00, 'enviado'),
    (3, 20.00, 'pagado'),  (3, 300.00, 'pendiente');

-- 4. OTROS (Clientes casuales, 1 orden c/u)
INSERT INTO ordenes (usuario_id, total, status) VALUES
    (4, 99.98, 'pendiente'),   -- Linus
    (5, 1299.99, 'pagado'),    -- Margaret
    (6, 399.99, 'pagado');     -- Donald


-- B. DETALLES DE ÓRDENES
-- Importante: Debemos poblar los detalles para que los reportes de "Top Productos" y "Stock"
-- tengan datos coherentes. Asumiremos que los IDs de ordenes van del 1 al 28 (total insertado arriba).

INSERT INTO orden_detalles (orden_id, producto_id, cantidad, precio_unitario) VALUES
    -- Detalles originales de Ada (Orden 1)
    (1, 1, 1, 1299.99), (1, 2, 1, 29.99), (1, 3, 1, 89.99),
    -- Relleno para las órdenes extra de Ada (Órdenes 2-12) usando productos variados
    (2, 6, 2, 19.99), (3, 5, 1, 59.99), (4, 11, 2, 34.99), (5, 14, 1, 19.99),
    (6, 13, 3, 24.99), (7, 4, 1, 399.99), (8, 10, 1, 14.99), (9, 3, 1, 89.99),
    (10, 2, 5, 29.99), (11, 8, 1, 39.99), (12, 7, 2, 49.99),

    -- Detalles originales de Alan (Orden 13)
    (13, 6, 2, 19.99), (13, 5, 1, 59.99),
    -- Relleno para órdenes extra de Alan (Órdenes 14-20)
    (14, 15, 1, 44.99), (15, 9, 1, 69.99), (16, 12, 1, 249.99), (17, 1, 1, 1299.99),
    (18, 5, 2, 59.99), (19, 11, 1, 34.99), (20, 6, 3, 19.99),

    -- Detalles originales de Grace (Orden 21)
    (21, 12, 1, 249.99), (21, 11, 1, 34.99),
    -- Relleno para órdenes extra de Grace (Órdenes 22-25)
    (22, 13, 2, 24.99), (23, 4, 1, 399.99), (24, 10, 2, 14.99), (25, 2, 1, 29.99),

    -- Detalles de los otros usuarios (Órdenes 26, 27, 28)
    (26, 7, 1, 49.99), (26, 8, 1, 39.99),  -- Linus
    (27, 1, 1, 1299.99),                   -- Margaret
    (28, 4, 1, 399.99);                    -- Donald
-- ============================================
